...
```rust
impl ComponentV2 for BadComponent {
    fn render(&self, area: Rect, buffer: &mut Buffer) {
        let (show_extra, _) = use_state_v2(|| false);

        // DON'T DO THIS - conditional hook call
        if show_extra {
            let (extra, _) = use_state_v2(|| 0); // Hook order changes!
        }
...
```rust
impl ComponentV2 for GoodComponent {
    fn render(&self, area: Rect, buffer: &mut Buffer) {
        let (show_extra, _) = use_state_v2(|| false);
        let (extra, _) = use_state_v2(|| 0);      // Always called
        let (count, _) = use_state_v2(|| 0);      // Always called
...
impl ComponentV2 for BadComponent {
    fn render(&self, area: Rect, buffer: &mut Buffer) {
        let (items, _) = use_state_v2(|| vec![1, 2, 3]);

        // DON'T DO THIS - variable number of hooks
        for item in &items {
            let (item_state, _) = use_state_v2(|| 0); // Hook count varies!
        }
    }
}
```
...
impl ComponentV2 for GoodComponent {
    fn render(&self, area: Rect, buffer: &mut Buffer) {
        ...
        let (item_states, set_item_states) = use_state_v2(|| {
...
impl ComponentV2 for BadComponent {
    fn render(&self, area: Rect, buffer: &mut Buffer) {
        let (mode, _) = use_state_v2(|| "state");

        // DON'T DO THIS - different hook types at same position
        if mode == "state" {
            let (value, _) = use_state_v2(|| 0);
        } else {
            let value = use_memo_v2(|| 0, ()); // Different hook type!
        }
    }
}
```
...
WARN fiber_id=FiberId(1) hook_index=1 previous_hook="use_state_v2" current_hook="use_effect_v2"
...
// ❌ Bad
for i in 0..n {
    let (item, _) = use_state_v2(|| 0);
}
```

### 2. Only Call Hooks from Components

Hooks can only be called within `ComponentV2::render()`.

```rust
// ✅ Good
impl ComponentV2 for MyComponent {
    fn render(&self, area: Rect, buffer: &mut Buffer) {
        let (state, _) = use_state_v2(|| 0); // Inside render
    }
}

// ❌ Bad
fn helper_function() {
    let (state, _) = use_state_v2(|| 0); // Outside component!
}
```
...
let (a, _) = use_state_v2(|| 0);
let (b, _) = use_state_v2(|| 0);
let (c, _) = use_state_v2(|| 0);
...
// ❌ Bad
if show_details {
    let (details, _) = use_state_v2(|| load_details());
    render_details(details);
}

// ✅ Good
let (details, _) = use_state_v2(|| None);
if show_details {
...
// ❌ Bad
for item in items {
    let (state, _) = use_state_v2(|| item.default());
}

// ✅ Good
let (states, set_states) = use_state_v2(|| {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    render_v2_with_options(|| App, options).await?;
    Ok(())
}
```
