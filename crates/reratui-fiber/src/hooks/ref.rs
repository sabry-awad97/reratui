...
//!
//! This module provides React-like `use_ref_v2` hook with proper fiber-based semantics.
//! Unlike `use_state_v2`, mutations to refs do NOT trigger re-renders.
//!
//! # Example
//!
//! ```rust,ignore
//! use reratui_fiber::hooks::use_ref_v2;
//!
//! #[component]
//! fn Counter() -> Element {
...
///
/// This struct is returned by `use_ref_v2` and provides methods to access and mutate
/// the stored value. Unlike `StateSetterV2`, mutations do NOT mark the fiber as dirty
/// and do NOT trigger re-renders.
///
/// # Thread Safety
///
/// `RefV2` is thread-safe and can be safely shared across async tasks.
...
impl<T: Clone + Send + 'static> RefV2<T> {
    /// Get a clone of the current value.
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let count_ref = use_ref_v2(|| 0);
    ...
    pub fn get(&self) -> T {
        self.storage.read().expect("RefV2 lock poisoned").clone()
    }
    ...
    pub fn set(&self, value: T) {
        let mut guard = self.storage.write().expect("RefV2 lock poisoned");
        *guard = value;
    }
    ...
    ///
    /// ```rust,ignore
    /// let data_ref = use_ref_v2(|| vec![1, 2, 3]);
    ...
    where
        F: FnOnce(&T) -> R,
    {
        let guard = self.storage.read().expect("RefV2 lock poisoned");
        f(&*guard)
    }
...
impl<T: Clone + Send + Default + 'static> RefV2<T> {
...
/// Unlike `use_state_v2`, mutations to refs do NOT trigger re-renders, making them ideal for:
///
...
/// # Thread Safety
///
/// The returned `RefV2` is thread-safe and can be safely shared across async tasks.
///
/// ## Comparison with `use_state_v2`
///
/// | Feature | `use_ref_v2` | `use_state_v2` |
/// |---------|--------------|----------------|
...
/// # Returns
///
/// A `RefV2<T>` that provides methods to access and mutate the value.
///
/// # Example
///
/// ```rust,ignore
...
/// #[component]
/// fn Timer() -> Element {
///     let interval_id = use_ref_v2(|| None::<IntervalHandle>);
///
///     use_effect_v2(|| {
...
pub fn use_ref_v2<T, F>(initializer: F) -> RefV2<T>
where
    ...
    F: FnOnce() -> T,
{
    ...
    .expect("use_ref_v2 must be called within a component render context")
}
...
mod tests {
    ...
    #[test]
    fn test_use_ref_v2_initial_value() {
        let _fiber_id = setup_test_fiber();

        let ref_handle = use_ref_v2(|| 42);
    ...
    #[test]
    fn test_use_ref_v2_returns_same_value_on_rerender() {
        let fiber_id = setup_test_fiber();

        // First render
        let ref_handle1 = use_ref_v2(|| 100);
        ...
        let ref_handle2 = use_ref_v2(|| 999);
    ...
    fn test_ref_with_accessor() {
        ...
        let ref_handle = use_ref_v2(|| vec![1, 2, 3]);
    ...
    fn test_multiple_refs() {
        ...
        let ref1 = use_ref_v2(|| 1);
        let ref2 = use_ref_v2(|| "hello".to_string());
        let ref3 = use_ref_v2(|| vec![true, false]);
    ...
    fn test_ref_persists_across_multiple_renders() {
        ...
        for i in 1..=5 {
            ...
            let ref_handle = use_ref_v2(|| 999); // Initializer should be ignored
    ...
    #[test]
    #[should_panic(expected = "use_ref_v2 must be called within a component render context")]
    fn test_use_ref_v2_panics_outside_render() {
        clear_fiber_tree();
...
mod property_tests {
    ...
    proptest! {
        ...
        //
        // For any ref created with `use_ref_v2` and any sequence of mutations,
        ...
        fn prop_ref_value_persistence_across_renders(
            ...
            for render_num in 1..=num_renders {
                ...
                let ref_handle = use_ref_v2(|| 999999);
        ...
        //
        // For any component using `use_ref_v2`, when the ref value is mutated
        ...
        fn prop_ref_with_mut_does_not_mark_dirty(
            ...
            let ref_handle = use_ref_v2(|| initial_vec);
